...
func GetByProperty_codeAndRound_CountID(Property_code string, Round_CountID string) ([]*model.Asset_check, error) {
	db := database.DB
	var assets []*model.Asset_check
	tx := db.Preload(clause.Associations).Where("property_code = ?", Property_code).Where("round_count_id = ?", Round_CountID).Find(&assets)
	if tx.Error != nil {
		return nil, tx.Error
	}
	return assets, nil
