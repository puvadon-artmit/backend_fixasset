package autoclik_photos_checkController
...
import (
	...
	"github.com/puvadon-artmit/gofiber-template/api/autoclik_photos_check/services"
...
// 	value, err := services.GetById(c.Params("autoclik_photos_check_id"))
// 	if err != nil {
...
func SaveFileDataToDB(filename string) error {
	...
	autoclik_photos_checkID := uuid.New().String()
	autoclik_photos_check := model.Autoclik_Photos_check{
		Autoclik_Photos_checkID: autoclik_photos_checkID,
		Photos:                  filename,
	}

	if err := database.DB.Create(&autoclik_photos_check).Error; err != nil {
		return err
	}
...
func Create(c *fiber.Ctx) error {
	autoclik_photos_check := new(model.Autoclik_Photos_check)
	err := c.BodyParser(autoclik_photos_check)
	if err != nil {
	...
	err = validator.New().Struct(autoclik_photos_check)
	if err != nil {
	...
	createdStatus, err := services.CreateNewAutoclik_Photos_check(*autoclik_photos_check)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func GetByIdHandler(c *fiber.Ctx) error {
	photosCheckID := c.Params("autoclik_photos_check_id")
...
func DeletePhotos(c *fiber.Ctx) error {
	...
	err = services.DeleteAutoclik_Photos_check(photosCheck)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
