...
func CountCategory() (count int, err error) {
...
func GetByCategory_Photo(id string) (CategoryImageURL string, err error) {
	...
	tx := db.Select("category_image").First(&categoryimage)
	if tx.Error != nil {
		return "", tx.Error
	}
...
func UpdateCategory(id string, updatedType model.Category) (value *model.Category, Error error) {
	db := database.DB
	Updategetcategory := model.Category{CategoryID: id}
...
func DeleteCategory(CategoryID string) error {
	db := database.DB
	tx := db.Where("category_id = ?", CategoryID).Delete(&model.Category{})
	if tx.Error != nil {
		return tx.Error
	}
	return nil
