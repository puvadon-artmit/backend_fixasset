package maliwan_photos_checkController
...
import (
	...
	"github.com/puvadon-artmit/gofiber-template/api/maliwan_photos_check/services"
...
// 	value, err := services.GetById(c.Params("maliwan_photos_check_id"))
// 	if err != nil {
...
func SaveFileDataToDB(filename string) error {
	...
	maliwan_photos_checkID := uuid.New().String()
	maliwan_photos_check := model.Maliwan_Photos_check{
		Maliwan_Photos_checkID: maliwan_photos_checkID,
		Photos:                 filename,
	}

	if err := database.DB.Create(&maliwan_photos_check).Error; err != nil {
		return err
	}
...
func Create(c *fiber.Ctx) error {
	maliwan_photos_check := new(model.Maliwan_Photos_check)
	err := c.BodyParser(maliwan_photos_check)
	if err != nil {
	...
	err = validator.New().Struct(maliwan_photos_check)
	if err != nil {
	...
	createdStatus, err := services.CreateNewMaliwan_Photos_check(*maliwan_photos_check)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func GetByIdHandler(c *fiber.Ctx) error {
	photosCheckID := c.Params("maliwan_photos_check_id")
...
func DeletePhotos(c *fiber.Ctx) error {
	...
	err = services.DeleteMaliwan_Photos_check(photosCheck)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
