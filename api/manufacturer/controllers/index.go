...
func UpdateManufacturer(c *fiber.Ctx) error {
	id := c.Params("manufacturer_id")

	updatedType := new(model.Manufacturer)
	...
	updatedStatus, err := ServicesManufacturer.UpdateManufacturer(id, *updatedType)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func GetCountManufacturer(c *fiber.Ctx) error {
	count, err := ServicesManufacturer.CountAllRecords()
	if err != nil {
		return c.Status(404).JSON(fiber.Map{
			"status":  "error",
			"message": "Not found!",
