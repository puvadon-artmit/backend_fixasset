...
func PullDataByBranch(c *fiber.Ctx, Maliwan_Counting_TriggerID string) error {
	maliwanCountingTrigger, err := ServicesMaliwan_Counting_Trigger.GetById(c.Params(Maliwan_Counting_TriggerID))
	if err != nil {
	...
	_, err = PullDatamaliwan(ArrayofCompanyName)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func PullDatamaliwan(branchMaliwanCounts []string) (result []model.Itemapivalue, err error) {
	Client := http.Client{
	...
	for _, branchMaliwanCount := range branchMaliwanCounts {
		...
		ItemApi := "/ItemsAPI"
		// filter := "Gen_Prod_Posting_Group eq 'PARTS' and Inventory gt 0"
...
func GetByTrigger_ID(c *fiber.Ctx, Maliwan_Counting_TriggerID string) error {
	...
	err = FilterData(c, maliwanCountID)
	if err != nil {
		fmt.Printf("Error maliwanCountID in maliwanCountID: %v\n", err)
	}

	return nil
}

func FilterData(c *fiber.Ctx, maliwanCountID string) error {
	...
	err = GetAutoByTrigger_ID(c, ArrayofCompanyName, Gen_prod_posting_Group_Maliwan, itemCategoryCodeList, maliwanCountID)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func GetAutoByTrigger_ID(c *fiber.Ctx, branchMaliwanCounts []string, Gen_prod_posting_Group string, itemCategoryCodes []string, maliwanCountID string) error {
	Client := http.Client{
		Transport: &httpntlm.NtlmTransport{
			Domain:   viper.GetString("nav.domain"),
			User:     viper.GetString("nav.username"),
