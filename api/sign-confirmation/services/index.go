...
func GetByAsset_countID(Asset_countID string) ([]*model.Signature, error) {
	db := database.DB
	var round_count []*model.Signature
	tx := db.Preload(clause.Associations).Where("asset_count_id = ?", Asset_countID).Find(&round_count)
	if tx.Error != nil {
		return nil, tx.Error
	}
