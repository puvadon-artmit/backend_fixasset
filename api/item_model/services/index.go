...
func UpdatebyItemModelByID(id string, updatedType model.Item_model) (value *model.Item_model, Error error) {
	db := database.DB
	existingType := model.Item_model{ItemModelID: id}
	tx := db.Model(&existingType).Where("item_model_id = ?", id).Updates(updatedType)
	if tx.Error != nil {
		return nil, tx.Error
	}
