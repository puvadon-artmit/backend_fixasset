...
func GetAll() (data *[]RoleResponse.GetAllResult, Error error) {
	...
	tx := db.Table("roles").Select("role_id, role_name, role_display_name, role_description, created_at, role_code").Scan(&result)
	if tx.Error != nil {
		return nil, tx.Error
	}
	return &result, nil
...
func GetroleAll() (result []*model.Role, err error) {
	db := database.DB
