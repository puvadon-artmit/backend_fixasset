...
func GetById(c *fiber.Ctx) error {
	value, err := ServicesAutoclik_Fixed_Asset_Count.GetById(c.Params("autoclik_fixed_asset_count_id"))
	if err != nil {
...
func GetAllHandler(c *fiber.Ctx) error {
	records, err := ServicesAutoclik_Fixed_Asset_Count.GetAllRecords()
	if err != nil {
		return c.Status(404).JSON(fiber.Map{
			"status":  "error",
			"message": "Not found!",
...
func Create(c *fiber.Ctx) error {
	count_autoclik := new(model.Autoclik_Fixed_Asset_Count)
	...
	createdStatus, err := ServicesAutoclik_Fixed_Asset_Count.CreateNewAutoclik_Fixed_Asset_Count(*count_autoclik)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func UpdateAutoclik_Fixed_Asset_Count(c *fiber.Ctx) error {
	id := c.Params("autoclik_fixed_asset_count_id")

	updatedCount_autoclik := new(model.Autoclik_Fixed_Asset_Count)
	...
	updatedStatus, err := ServicesAutoclik_Fixed_Asset_Count.UpdateAutoclik_Fixed_Asset_Count(id, *updatedCount_autoclik)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
...
func DeleteAutoclik_Fixed_Asset_Count(c *fiber.Ctx) error {
	BranchStoryID := c.Params("autoclik_fixed_asset_count_id")

	count_autoclik, err := ServicesAutoclik_Fixed_Asset_Count.GetById(BranchStoryID)
	if err != nil {
		return c.Status(404).JSON(fiber.Map{
			"status":  "error",
			"message": "Not found!",
	...
	err = ServicesAutoclik_Fixed_Asset_Count.DeleteAutoclik_Fixed_Asset_Count(count_autoclik)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status": "error",
			"error":  err.Error(),
