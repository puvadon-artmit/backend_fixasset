...
func UpdateGroundByID(id string, updatedItem model.Ground) error {
	db := database.DB
...
func UpdateGround(id string, updatedType model.Ground) (value *model.Ground, Error error) {
	db := database.DB
	existingType := model.Ground{GroundID: id}
	tx := db.Model(&existingType).Where("ground_id = ?", id).Updates(updatedType)
	if tx.Error != nil {
		return nil, tx.Error
	}
...
func DeleteGroundID(ground *model.Ground) error {
	db := database.DB
	tx := db.Delete(ground)
	if tx.Error != nil {
		return tx.Error
